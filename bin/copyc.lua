...
function command(c, noreturn)
	os.execute(c.." > temp\\result.txt&@fdoswin /off temp\\result.txt")
	local listfile=io.open("temp/result.txt","r")
...
function getfilehash(fileinfo)
	os.execute("@rhash.cmd \""..fileinfo.path.."\" > temp\\result.txt")
	local newfileinfo = loadfile("temp\\result.txt")()
...
function getfilesize(fileinfo)
	local listfile=io.open(fileinfo.path,"rb")
	if listfile then
...
hl=io.open("hardlinker.cmd","w")
print("----------------------------------------------------------------")
freebytes=0
for hash, files1 in pairs(copylists) do
	local line = "\nHASH: "..hash.."\nSIZE: "..files1[1].size
	local masterfile = nil
	local idx = 1
